  ...
  <body>
    ...
    <?php
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["username"])
      {
        ...
        if ($numHoles == 9)
        {
          $query = $conn->query(sprintf(EDIT_9HOLE_COURSE, $clubName, $stateLocation, $countryLocation, $yardageUnits, $par1, $par2, $par3, $par4, $par5, $par6, $par7, $par8, $par9, $courseId));
        }
        else
        {
          $query = $conn->query(sprintf(EDIT_18HOLE_COURSE, $clubName, $stateLocation, $countryLocation, $yardageUnits, $par1, $par2, $par3, $par4, $par5, $par6, $par7, $par8, $par9, $par10, $par11, $par12, $par13, $par14, $par15, $par16, $par17, $par18, $courseId));
        }
        ...
        if ($query)
        {
          ...
          $yardageResults = $conn->query(sprintf(GET_YARDAGES, $courseId));
          $numYardages = $yardageResults->num_rows;
          for ($i = 0; $i < $numTeeBoxes; $i++)
          {
            if ($i < $numYardages)
            {
              $yardage = $yardageResults->fetch_assoc();
              $yardageId = $yardage["yardageID"];

              if ($numHoles == 9)
              {
                $query = $conn->query(sprintf(EDIT_9HOLE_YARDAGE, $distance1[$i], $distance2[$i], $distance3[$i], $distance4[$i], $distance5[$i], $distance6[$i], $distance7[$i], $distance8[$i], $distance9[$i], $yardageId));
              }
              else
              {
                $query = $conn->query(sprintf(EDIT_18HOLE_YARDAGE, $distance1[$i], $distance2[$i], $distance3[$i], $distance4[$i], $distance5[$i], $distance6[$i], $distance7[$i], $distance8[$i], $distance9[$i], $distance10[$i], $distance11[$i], $distance12[$i], $distance13[$i], $distance14[$i], $distance15[$i], $distance16[$i], $distance17[$i], $distance18[$i], $yardageId));
              }

              if ($query)
              {
                $query = $conn->query(sprintf(EDIT_TEEBOX, $colour[$i], $courseId, $yardageId));
              }
            }
            else
            {
          ...
          if ($numTeeBoxes < $numYardages)
          {
            while($yardage = $yardageResults->fetch_assoc())
            {
              $yardageId = $yardage["yardageID"];

              $conn->query(sprintf(DELETE_TEEBOX, $courseId, $yardageId));
              $conn->query(sprintf(DELETE_YARDAGE, $yardageId));
            }
          }
        }
      }
    ...
    <?php
      ...
      {
        echo "<script>
                window.alert(\"Failed to update course! Please refresh the page and try again!\");
                document.getElementById(\"editSubmit\").click();
              </script>";
      }
    ?>
