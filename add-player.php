  ...
  <body>
    ...
    <?php
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["username"])
      {
        ...
        $query = $conn->query(sprintf(ADD_PLAYER, $firstName, $lastName, $dayOfBirth, $monthOfBirth, $yearOfBirth, $gender, $country, $email, $guardianName, $guardianMobileNumber, $lastUpdatedDay, $lastUpdatedMonth, $lastUpdatedYear));

        if ($query)
        {
          $playerAdded = $conn->query(GET_LAST_PLAYER_ADDED);
          $player = $playerAdded->fetch_assoc();
    ...
    <?php
      ...
      {
        echo "<script>
                window.alert(\"Player added successfully with Player ID: ", $playerId, "\");
      ...
      {
        echo "<script>
                window.alert(\"Failed to add player! Please refresh the page and try again!\");
                document.getElementById(\"addSubmit\").click();
              </script>";
      }
    ?>
